...
%           max_iter   : maximum number of training iterations (integer) 10
%           epsilon    : minimum improvement for iteration (float) 0.001
%           M          : number of Gaussians/mixture (integer) 8
%
...
for folder=3:length(DD)
    ...
    path = [dir_train, filesep, DD(folder).name, filesep];  % filesep = '/'
    D2 = dir( horzcat(path, ['*', 'mfcc']) );

    mfccMatrix = []; % It will contain all the Mfcc features written in the files in the Vertical Fashion

    for iFile=1:length(D2)
        mfccMatrix = vertcat(mfccMatrix, load(horzcat(path,[D2(iFile).name])))
    end

    [mfccMatrix, eigVecs, pcameans] = pca(mfccMatrix, 10); % It converts the 19 dimension vectors into 10 dimension vectors

    d = size(mfccMatrix,2);  % the value is 10
    omega = [];

    for m=1:M
        omega(:,:,m) = eye(d); %eye function just makes the Identity Matrix with d dimensions
    end
    % Intialising all the covariance matrices of all the Gaussians to the Identity matrices

    k = randperm(size(mfccMatrix,1));
    mus = mfccMatrix(k(1:M),:); % randomly choosing the first 8 vectors to be Means
    mus = mus';
